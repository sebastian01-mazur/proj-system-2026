...
function getAmount(value, fallback = 0) {
...
export default function Home() {
  ...
  function getPersonAvatar(name = "") {
    const safeName = getText(name, "");

    if (safeName.includes("Piotr")) return "👨🏻";
    if (safeName.includes("Joanna")) return "👩🏻";
    if (safeName.includes("Jan")) return "👨🏻‍💼";
    if (safeName.includes("Marek")) return "👨🏼";

    return "👤";
  }

  function mapTripStatus(status = "") {
    const safeStatus = getText(status, "");

    if (safeStatus === "PLANNED") return "Planowane";
    if (safeStatus === "IN_PROGRESS") return "W trakcie";
    if (safeStatus === "COMPLETED") return "Zakończone";
    if (safeStatus === "ACTIVE") return "W trakcie";
    if (safeStatus === "FINISHED") return "Zakończone";
    if (safeStatus === "Planowana") return "Planowane";

    return safeStatus || "Planowane";
  }

  function getTripTitle(trip, country, city) {
    const name = getText(
      trip.name || trip.tripName || trip.title || trip.nazwa,
      ""
    );
  ...
  function mapTrip(trip) {
    const country = getText(
      trip.country || trip.destinationCountry || trip.kraj,
      "Brak kraju"
    );

    const city = getText(
      trip.city || trip.destinationCity || trip.miasto,
      ""
    );

    const rawParticipants =
      trip.participants || trip.members || trip.tripParticipants || [];
    ...
    return {
      id: getId(trip.id || trip.tripId || trip.idPodrozy),
      ...
      startDate: getText(
        trip.startDate || trip.dateFrom || trip.dataRozpoczecia,
        ""
      ),
      endDate: getText(
        trip.endDate || trip.dateTo || trip.dataZakonczenia,
        ""
      ),
      budget: getAmount(
        trip.budget || trip.plannedBudget || trip.budzetPlanowany,
        0
      ),
      currency: getText(
        trip.currency || trip.baseCurrency || trip.walutaBazowa,
        "EUR"
      ),
      participants,
      participantsCount: getAmount(participantsCount, 0),
  ...
  function mapExpense(expense) {
    return {
      id: getId(expense.id || expense.expenseId || expense.idWydatku),
      tripId: getId(expense.tripId || expense.idPodrozy || expense.trip),
      category: getText(
        expense.category || expense.categoryName || expense.kategoria,
        "Wydatek"
      ),
      date: getText(expense.date || expense.expenseDate || expense.dataWydatku, ""),
      paidBy: getText(
        expense.paidBy || expense.payerName || expense.placacy || expense.payer,
        "Użytkownik"
      ),
      amount: getAmount(
        expense.amount || expense.originalAmount || expense.kwotaOryginalna,
        0
      ),
      currency: getText(
        expense.currency || expense.originalCurrency || expense.walutaOryginalna,
        "EUR"
      ),
      amountInBaseCurrency:
        ...
        null,
      baseCurrency: getText(
        expense.baseCurrency || expense.walutaBazowa,
        "PLN"
      ),
    };
  }
  ...
  const dashboardTrips = (
    dashboardData?.userTrips ||
    dashboardData?.trips ||
    dashboardData?.organizedTrips ||
    []
  ).map(mapTrip);
  ...
  const totalPlannedBudget = getAmount(
  ...
  const currency = getText(
    budgetStats.currency || budgetStats.baseCurrency || trips[0]?.currency,
    "EUR"
  );
  ...
  const defaultTripId = trips[0]?.id || recentExpenses[0]?.tripId || "";

  function getSafeTripId(tripId) {
    const safeTripId = getId(tripId, "");

    if (safeTripId && safeTripId !== "1") {
      return safeTripId;
    }
  ...
  return (
    <Layout>
      <main className="content home-content">
        ...
        {apiError && <Card className="auth-error">{apiError}</Card>}
        ...
        <section className="home-section">
          <div className="section-header">
            ...
            {defaultTripId ? (
              <Link
                to={`/trip/${defaultTripId}/expenses`}
                className="small-blue-btn"
              >
                + Dodaj wydatek
              </Link>
            ) : (
              <Link to="/new-trip" className="small-blue-btn">
                + Dodaj wydatek
              </Link>
            )}
          </div>
          ...
          <div className="home-expenses-grid">
            ...
            {recentExpenses.map((expense) => (
              <Card className="home-expense-card" key={expense.id}>
                ...
                {getSafeTripId(expense.tripId) && (
                  <Link
                    to={`/trip/${getSafeTripId(expense.tripId)}/expenses`}
        ...
        <section className="home-section">
          ...
          <Card className="spending-stats-card">
            <div className="stats-card-header">
              ...
              {defaultTripId ? (
                <Link
                  to={`/trip/${defaultTripId}/reports`}
                  className="stats-open-link"
                >
                  ↗
                </Link>
              ) : (
                <Link to="/new-trip" className="stats-open-link">
                  ↗
                </Link>
              )}
            </div>
